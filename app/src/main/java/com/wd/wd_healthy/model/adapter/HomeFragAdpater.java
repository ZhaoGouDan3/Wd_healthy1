 ...
 * @date 2022/9/12
 */
public class HomeFragAdpater extends RecyclerView.Adapter<RecyclerView.ViewHolder>{


    private List<ZxBean.ResultBean> list;
    private Context context;
    SimpleDateFormat sdf=new SimpleDateFormat("yyyy年MM月dd日");

    public HomeFragAdpater(List<ZxBean.ResultBean> list, Context context) {
        this.list = list;
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if(list.get(position).getThumbnail().split(";").length>1){
            ...
            binding.homeItem2Name.setText(list.get(position).getSource());
            ...
            binding.homeItem1Name.setText(list.get(position).getSource());
